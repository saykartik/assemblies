...
EMBEDS_FN = '../data/glove.twitter.27B.100d.txt'
...
def get_tokens(data):
    ...
    """
    # can do some optional preprocessing using TweetTokenizer
    # tknzr = TweetTokenizer() # Create TweetTokenizer
    # return [tknzr.tokenize(datapoint.lower()) for datapoint in data['content'].tolist()] # Use TweetTokenizer to tokenize
...
def get_data(data_fn):
    ...
    # .sample() shuffles the data - set the random seed so we always grab the same data split(setting seed is optional)
...
def vectorize_data(train, dev, test, batch_size, embedding_dim, create_embedding = True):
    """
    Transform the data from pandas DataFrame form into numerical data packed in DataLoaders.
    (Also create the pretrained embedding matrix in the proper order).
    ...
    :param create_embedding: Whether to create embeddings or not
    :return: train_generator, dev_generator, test_generator (DataLoaders for each dataset),
             ...
             test_data (the Dataset corresponding to the test set, including the word2idx dictionary and label encoder
    """
    ...
    if create_embedding:
        ...
        embeddings = None
    return train_generator, dev_generator, test_generator, embeddings, train_data, test_data
