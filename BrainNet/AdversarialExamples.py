...
def adversarial_example(x, y, net, eps=100, lr=1e-2,verbose=False):
    ...
    while True:
        ...
        if verbose:
            print("Loss/cnt/eps_curr/Y:", loss.item(), cnt, np.linalg.norm(inp.data - original.data),torch.argmax(outputs).item())
...
def epsilon_sensitivity_trend(models, orig_test_data, num_labels):
    ...
    for model_name, model in models.items():
        ...
        for epsilon in epsilons:
            this_model = model
            X, Y = get_adv_bulk_data(new_data, this_model.forward_pass, epsilon)
            acc = evaluate(X, Y, num_labels, this_model.forward_pass)
