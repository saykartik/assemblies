...
class OneBetaANNRule_PostAll(OneBetaANNPlasticityRule):
    """
    This class implements the following ANN-based plasticity rule:

    The rule used to update a synapse between hidden-layer nodes i and j is an ANN with 1+<hidden-layer width> input features:
    ...
    The rule used to update a synapse between hidden-layer node i and output label j is an ANN with 1+<hidden-layer width> input features:
    ...
    def rule_size(self):
        return 1+self.presynaptic_width, 20, 1      # Input size, Hidden layer size, Output size
